...
**Be 100% accurate.** Subaward agreements have **character-perfect** requirements for emails, phone numbers, dollar amounts, and dates — quote them verbatim. When a field is not specified, set it to `null` or — for arrays — return an empty array. Do not invent values.
...
- `federal_award_number` — PTE's Federal Award Number (FAIN). String or `null`.
- `subaward_number` — Subaward number. Required.
- `project_title` — full project title. Required.
- `federal_awarding_agency` — federal awarding agency name. String or `null`.
...
**Financial policies:**
- `invoicing_frequency` — one of `"Monthly"`, `"Quarterly"`, `"Semi-Annual"`, `"Annual"`, or `null`.
...
2. **`amount_funded == total_direct_costs + total_indirect_costs`** when all three are present (downstream cross-field check `CHK-01`). When the agreement does not reconcile, preserve the document's reported values.
